...
type rxBidExt struct {
	Rx rxBidExtRx `json:"rx"`
}

type rxBidExtRx struct {
	BidType int `json:"bid_ad_type"`
}

type rxImpExt struct {
	Rx rxImpExtRx `json:"rx"`
}
...
func (a *RxAdapter) MakeBids(internalRequest *openrtb.BidRequest, externalRequest *adapters.RequestData, response *adapters.ResponseData) ([]*adapters.TypedBid, []error) {
...
func getMediaTypeForBid(bid *openrtb.Bid) (openrtb_ext.BidType, error) {
	...
	default:
		return "", fmt.Errorf("Unrecognized bid_ad_type in response from appnexus: %d", impExt.Rx.BidType)
	}
}
